...
public class DriverLocationDAO extends BaseDaoImpl<DriverLocation, Integer> {

    public static String[] minimumColumnSet = {"bookingId"};

    public DriverLocationDAO(ConnectionSource connectionSource, Class<DriverLocation> dataClass)
            throws SQLException {
        super(connectionSource, dataClass);
    }


    public DriverLocation getLast(String bookingId) throws Exception {
        QueryBuilder<DriverLocation, Integer> queryBuilder = queryBuilder();
        Where<DriverLocation, Integer> where = queryBuilder.where();
        where.eq("bookingId", bookingId);
        queryBuilder.orderBy("capturedTime", false);
        DriverLocation driverLocation = where.queryForFirst();
        return driverLocation;
    }

    public DriverLocation getDriverLocationOfCheckpoint(String bookingId,String checkpoint) throws Exception{
        QueryBuilder<DriverLocation, Integer> queryBuilder = queryBuilder();
        Where<DriverLocation, Integer> where = queryBuilder.where();
        where.and(where.eq("bookingId",bookingId),where.eq("checkpoint",checkpoint));
        queryBuilder.orderBy("capturedTime", false);
        DriverLocation driverLocation = where.queryForFirst();
        return driverLocation;
    }
