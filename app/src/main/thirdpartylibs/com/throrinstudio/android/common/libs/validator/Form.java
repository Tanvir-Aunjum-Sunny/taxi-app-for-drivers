...
public class Form {
    ...
    private Set<AbstractValidate> mValidates = new HashSet<AbstractValidate>();
    ...
    public boolean validate() {
        ...
        for (AbstractValidate validate : mValidates) {
            formValid = formValid & validate.isValid(); // Use & in order to
            // evaluate both side of
            // the operation.
        }
        return formValid;
    }
    ...
    /**
     * Closes the error popup of the text view. A little useless due to ability
     * to just call source.setError(null), but added anyways
     *
    ...
    public void closeAllErrors() {
        for (AbstractValidate av : mValidates) {
            Validate v = (Validate) av;
            if (v.getSource() instanceof TextView)
                v.getSource().setError(null);
        }
    }
}
